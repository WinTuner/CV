			...
			<div className="mx-auto max-w-5xl px-4 space-y-24">
				...
				<section id="experience" className="space-y-12">
					...
					<div className="grid gap-12">
						{(
							Object.keys(professionalExperience[language]) as Array<
								keyof typeof t.categories
							>
						).map((cat) => (
							<div key={cat} className="space-y-6">
								...
								<div className="grid gap-6 md:grid-cols-2">
									{professionalExperience[language][cat].map((proj) => (
										<article
											key={proj.name}
											className="group relative rounded-xl border border-border/50 bg-card/30 p-6 glass transition-all duration-300 hover:border-primary/40 hover:bg-card/50"
										>
						...
						<div className="rounded-xl border border-border/50 bg-card/30 p-6 space-y-6">
							...
							<ul className="space-y-4">
								{selfDevelopment[language].certifications.map((cert) => (
									<li
										key={cert.name}
										className="group flex flex-col gap-1 border-l-2 border-primary/20 pl-4 transition-all hover:border-primary"
									>
						...
						<div className="rounded-xl border border-border/50 bg-card/30 p-6 space-y-6">
							...
							<ul className="space-y-4">
								{selfDevelopment[language].workshops.map((ws) => (
									<li
										key={ws.name}
										className="group flex flex-col gap-1 border-l-2 border-border pl-4 transition-all hover:border-primary"
									>
				...
				<section className="space-y-12">
					...
					<div className="grid gap-6 md:grid-cols-2">
						{awards[language].competitions.map((comp) => (
							<div
								key={comp.name}
								className="flex flex-col gap-4 rounded-xl border border-primary/20 bg-primary/5 p-6 transition-transform hover:scale-[1.02]"
							>
						...
						{awards[language].honors.map((hon) => (
							<div
								key={hon.name}
								className="flex items-center gap-5 rounded-xl border border-border/50 bg-card/30 p-6"
							>
				...
				<section className="space-y-12">
					...
					<div className="grid gap-8">
						{leadership[language].map((item) => (
							<article
								key={item.title}
								className="relative overflow-hidden rounded-xl border border-border/50 bg-card/50 p-8 glass"
							>
									...
									<div className="space-y-3">
										...
										<div className="flex flex-wrap gap-2">
											{item.softSkills.map((skill) => (
												<span
													key={skill}
													className="rounded border border-primary/20 bg-primary/5 px-2.5 py-1 text-[10px] font-medium text-primary"
												>
													{skill}
												</span>
											))}
				...
				<section className="pt-12 grid gap-12 md:grid-cols-2 border-t border-border/30">
					<div className="space-y-8">
						...
						<div className="space-y-6">
							{education[language].map((item) => (
								<div
									key={item.school}
									className="space-y-2 relative pl-6 before:absolute before:left-0 before:top-2 before:h-2 before:w-2 before:rounded-full before:bg-primary"
								>
					...
					<div className="space-y-8">
						<h3 className="text-2xl font-bold flex items-center gap-3">
							...
							{language === "en" ? "Work" : "ประสบการณ์ทำงาน"}
						</h3>
						<div className="space-y-6">
